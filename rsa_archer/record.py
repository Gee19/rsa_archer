...
class Record:
    ...
    def __init__(self, archer_instance, json):
        ...
        self.json = json
        self.record_sequential_id = json[
            "SequentialId"
        ]  # just a sequesntial number of record created in application, in my case it was used for uniquly identify a record

    def get_field_content(self, field_name):
        """
        :param field_name: name how you see it in app
        :returns value of the field array of Users for user field
         - Note: LIST of values for values list, including parent value in leveled values list. Looks like this [Parent Value:Value]
        ...
        try:
            if (
                field_type == 4
            ):  # Values List return ValuesListIds, which should be returned {'ValuesListIds': [69809], 'OtherText': None}
                values_list_id = self.archer_instance.get_vl_id_by_field_name(
                    field_name
                )
                list_of_value_ids = self.json["FieldContents"][str(field_id)]["Value"][
                    "ValuesListIds"
                ]
                ...
                if len(list_of_value_ids) > 1:
                    ...
                    for one_id in list_of_value_ids:
                        returned_value = self.get_value_from_valueslistid(
                            one_id, values_list_id
                        )
                        multiple_values.append(returned_value)

                    return multiple_values
                else:
                    value_id = self.json["FieldContents"][str(field_id)]["Value"][
                        "ValuesListIds"
                    ][0]
            ...
            if (
                field_type == 8
            ):  # User/group list {'UserList': [{'Id': 11077, 'HasRead': True, 'HasUpdate': True, 'HasDelete': False}], 'GroupList': []}
                user_ids = self.json["FieldContents"][str(field_id)]["Value"][
                    "UserList"
                ]
                users = []
    ...
    def get_value_from_valueslistid(self, value_id, values_list_id):
        ...
        """
        api_url = (
            self.archer_instance.api_url_base
            + "core/system/valueslistvalue/flat/valueslist/"
            + str(values_list_id)
        )
            ...
            for value in data:
                if value["RequestedObject"]["Id"] == value_id:
                    if value["RequestedObject"]["ParentId"]:
                        ...
                        return (
                            parent_value_in_multi_layer_setup
                            + ":"
                            + value["RequestedObject"]["Name"]
                        )
                    else:
